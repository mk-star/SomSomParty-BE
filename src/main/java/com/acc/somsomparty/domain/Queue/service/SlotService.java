...
public class SlotService {
    ...
    // 축제 모니터링이 끝나면 슬롯을 해제함
    public Mono<Void> releaseSlot(String festivalId) {
        String activeKey = SLOT_ACTIVE_KEY.formatted(festivalId);
        ...
        return reactiveRedisTemplate.execute(script, List.of(activeKey))
                .doOnNext(success -> {
                    if (success) {
                        log.info("슬롯 해제 성공 [Festival: {}]", festivalId);
                    } else {
                        log.warn("슬롯 해제 실패 (슬롯이 이미 0) [Festival: {}]", festivalId);
                    }
                }).then();
    }
}
